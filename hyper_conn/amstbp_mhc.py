"""
AMSTBP-mHC: Averaged Margined Scaled Transport Birkhoff Polytope Manifold-Constrained Hyper-Connections

This module implements the averaged chart mixture for MSTBP-mHC. It runs the margined
scaled transport sequential Birkhoff algorithm with different row/column orderings
(permutations), then averages the resulting doubly stochastic matrices with
learnable weights. This reduces the sequential bias while keeping the margined
scaled transport parameterization for improved gradient flow and numerical stability.

===============================================================================
MOTIVATION
===============================================================================

MSTBP-mHC uses X[i,j] = L_ij + (U_ij - L_ij) * (ρ + (1-2ρ)*sigmoid(β*t/(U_ij - L_ij + ε))).
The sequential algorithm still privileges certain rows/columns. By averaging
...
from .mstbp_mhc import margined_sequential_birkhoff


# ============================================================================
# PERMUTATION HELPERS (shared with ALSB/ASTBP)
# ============================================================================
...
# ============================================================================
# AVERAGED MARGINED SCALED TRANSPORT BIRKHOFF
# ============================================================================

def averaged_margined_sequential_birkhoff(
    ...
    *,
    ...
    Uses MSTBP formula: X[i,j] = L + (U-L)*(ρ + (1-2ρ)*sigmoid(β*t/(U-L+ε))).
    """
    K = len(permutations)
    ...
    for k in range(K):
        ...
        S_k = margined_sequential_birkhoff(t_flat, delta_logit=None, eps=eps)
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs
    ...
    """Get initializer for AMSTBP-mHC layers plus expand/reduce functions."""
...
# ============================================================================
# AMSTBP-mHC IMPLEMENTATION
# ============================================================================

class AMSTBP_MHC(BaseHyperConnections):
    """
    AMSTBP-mHC: Averaged Margined Scaled Transport Birkhoff Polytope.

    Uses K permutations with MSTBP formula (margined scaled sigmoid), each with its own
    (n-1)² parameters, and learnable weights to average the resulting doubly
    stochastic matrices.
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert num_residual_streams >= 2, "AMSTBP-mHC requires at least 2 streams"
    ...
    def _init_hyper_params(self):
        """Initialize AMSTBP-mHC parameters: K charts × (n-1)² params, chart weights, H^pre, H^post."""
    ...
    def _compute_alpha_beta(self, normed: torch.Tensor, device: torch.device):
        """Compute H^pre, averaged H^res (MSTBP), and H^post."""
        n = self.num_residual_streams
        f = self.num_fracs
        v = self.num_input_views
        K = self.K
        ...
        for g in range(f):
            ...
            ds_g = averaged_margined_sequential_birkhoff(
                t_list,
                self.permutations,
                weights_g,
                n,
...
AMSTBP_MHC.get_expand_reduce_stream_functions = staticmethod(get_expand_reduce_stream_functions)
AMSTBP_MHC.get_init_and_expand_reduce_stream_functions = staticmethod(get_init_and_expand_reduce_stream_functions)
