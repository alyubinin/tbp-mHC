"""
ASTBP-mHC: Averaged Scaled Transport Birkhoff Polytope Manifold-Constrained Hyper-Connections

This module implements the averaged chart mixture for STBP-mHC. It runs the scaled
transport sequential Birkhoff algorithm with different row/column orderings
(permutations), then averages the resulting doubly stochastic matrices with
learnable weights. This reduces the sequential bias while keeping the scaled
transport parameterization for improved gradient flow.

===============================================================================
MOTIVATION
===============================================================================

STBP-mHC uses X[i,j] = L_ij + (U_ij - L_ij) * sigmoid(β*t/(U_ij - L_ij + ε)).
...
from .stbp_mhc import sequential_birkhoff


# ============================================================================
# PERMUTATION HELPERS (shared with ALSB)
# ============================================================================
...
# ============================================================================
# AVERAGED SCALED TRANSPORT BIRKHOFF
# ============================================================================

def averaged_sequential_birkhoff(
    ...
    *,
    ...
    Uses STBP (scaled transport) formula: X[i,j] = L + (U-L)*sigmoid(β*t/(U-L+ε)).
    """
    K = len(permutations)
    ...
    for k in range(K):
        ...
        S_k = sequential_birkhoff(t_flat, delta_logit=None, eps=eps)
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs
    ...
    """Get initializer for ASTBP-mHC layers plus expand/reduce functions."""
...
# ============================================================================
# ASTBP-mHC IMPLEMENTATION
# ============================================================================

class ASTBP_MHC(BaseHyperConnections):
    """
    ASTBP-mHC: Averaged Scaled Transport Birkhoff Polytope.

    Uses K permutations with STBP formula (scaled sigmoid), each with its own
    ...
    def __init__(
        ...
        *,
        ...
        assert num_residual_streams >= 2, "ASTBP-mHC requires at least 2 streams"
    ...
    def _init_hyper_params(self):
        """Initialize ASTBP-mHC parameters: K charts × (n-1)² params, chart weights, H^pre, H^post."""
    ...
    def _compute_alpha_beta(self, normed: torch.Tensor, device: torch.device):
        """Compute H^pre, averaged H^res (STBP), and H^post."""
        n = self.num_residual_streams
        f = self.num_fracs
        v = self.num_input_views
        K = self.K
        ...
        for g in range(f):
            ...
            ds_g = averaged_sequential_birkhoff(
                t_list,
                self.permutations,
                weights_g,
                n,
...
ASTBP_MHC.get_expand_reduce_stream_functions = staticmethod(get_expand_reduce_stream_functions)
ASTBP_MHC.get_init_and_expand_reduce_stream_functions = staticmethod(get_init_and_expand_reduce_stream_functions)
