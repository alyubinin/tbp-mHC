...
This module is the scaled-sigmoid analogue of RTBP-mHC. It keeps the same
recursive transportation-polytope construction, but every free interval choice

    x in [L, U]

is parameterized with the scaled transport formula

    x = L + (U - L) * sigmoid(beta * t / (U - L + epsilon))

instead of the plain chart

    x = L + (U - L) * sigmoid(t).

This mirrors the motivation of STBP-mHC, but applied to the recursive RTBP
chart rather than the sequential Birkhoff chart.
"""
...
def _choose_in_interval(
    ...
    *,
    eps: float = 1e-7,
    beta: float = SRTBP_BETA,
    epsilon: float = SRTBP_EPSILON,
) -> torch.Tensor:
    """Map unconstrained logits into [lower, upper] with the STBP-style scaling."""
    ...
    return lower + interval_width * torch.sigmoid(scaled_arg)
...
def _split_vector(
    ...
    *,
    ...
    """
    Batched differentiable SplitVector(m, w) using the scaled RTBP interval map.
...
def _recursive_transport_tbp(
    ...
    *,
    ...
    """
    Internal batched recursive TBP constructor for the scaled RTBP chart.
    ...
    if n % 2 == 0:
        ...
        if idx != expected:
            raise RuntimeError(f"SRTBP parameter bookkeeping mismatch for n={n}")
        ...
        stacked_x = _recursive_transport_tbp(
            stacked_theta,
            stacked_r,
            stacked_c,
            k,
    ...
    child = _recursive_transport_tbp(
        child_theta,
        row_prefix - q,
        col_prefix - p,
        last,
...
def scaled_recursive_transport_birkhoff(
    t: torch.Tensor,
    *,
    ...
    scaled recursive transportation-polytope parameterization.

    Args:
        t: (..., n-1, n-1) unconstrained parameters
        delta_logit: optional scalar/broadcastable logit for uniform minorization
        eps: small safety clamp for interval width
        beta: scale factor for the sigmoid argument
        epsilon: denominator offset for scaled transport
    ...
    X = _recursive_transport_tbp(theta, r, c, n, eps=eps, beta=beta, epsilon=epsilon)
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs,
    ...
    """Get initializer for SRTBP-mHC layers plus expand/reduce functions."""
    disable = default(disable, num_streams == 1 and num_fracs == 1)

    hyper_conn_klass = SRTBP_MHC if not disable else Residual
...
class SRTBP_MHC(BaseHyperConnections):
    """
    SRTBP-mHC: Scaled Recursive Transportation Birkhoff Polytope.

    Same recursive chart as RTBP-mHC, but every interval choice uses:

        x = L + (U-L) * sigmoid(beta * t / (U-L + epsilon))

    which mirrors STBP-mHC's scaled transport parameterization.
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert num_residual_streams >= 2, "SRTBP-mHC requires at least 2 streams"
    ...
    def _init_hyper_params(self):
        """Initialize SRTBP-mHC parameters: H^pre, H^res, H^post, delta."""
    ...
    def _compute_alpha_beta(self, normed: torch.Tensor, device: torch.device):
        """Compute H^pre, H^res (scaled recursive TBP), and H^post matrices."""
        n = self.num_residual_streams
        f = self.num_fracs
        v = self.num_input_views
        ...
        srtbp_params = alpha_res[..., : n - 1, : n - 1]

        orig_shape = srtbp_params.shape[:-2]
        srtbp_flat = srtbp_params.reshape(-1, n - 1, n - 1)

        ds_flat = scaled_recursive_transport_birkhoff(
            srtbp_flat,
...
SRTBP_MHC.get_expand_reduce_stream_functions = staticmethod(
    get_init_and_expand_reduce_stream_functions
)
SRTBP_MHC.get_init_and_expand_reduce_stream_functions = staticmethod(
    get_init_and_expand_reduce_stream_functions
)


class ScaledRecursiveTransportBirkhoff(nn.Module):
    """Standalone wrapper for scaled_recursive_transport_birkhoff()."""
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        return scaled_recursive_transport_birkhoff(
            t,
            delta_logit=self.delta_logit if self.make_dse else None,
            eps=self.eps,
            beta=self.beta,
