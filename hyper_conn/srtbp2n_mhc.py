...
This module is the scaled-sigmoid analogue of RTBP2N-mHC. It keeps the same
...
This mirrors the motivation of STBP-mHC, but applied to the power-of-2 RTBP2N
...
def _binary_split_vector(
    ...
    *,
    ...
    """
    Balanced binary split for vectors of power-of-2 length using the scaled RTBP2N
    interval map.
...
def _recursive_transport_tbp_power2(
    ...
    *,
    ...
    """
    Internal power-of-2 recursive TBP constructor for the scaled RTBP2N chart.
    ...
    if not _is_power_of_two(n):
        raise ValueError(f"Power-of-2 SRTBP2N requires n=2^L, got {n}")
    ...
    if idx != expected:
        raise RuntimeError(f"SRTBP2N parameter bookkeeping mismatch for n={n}")
...
def scaled_recursive_transport_birkhoff_power2(
    t: torch.Tensor,
    *,
    ...
    using the scaled power-of-2 recursive transportation-polytope parameterization.
    """
    if t.ndim < 2:
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs,
    ...
    """Get initializer for SRTBP2N-mHC layers plus expand/reduce functions."""
    disable = default(disable, num_streams == 1 and num_fracs == 1)

    hyper_conn_klass = SRTBP2N_MHC if not disable else Residual
...
class SRTBP2N_MHC(BaseHyperConnections):
    """
    SRTBP2N-mHC: scaled power-of-2 recursive transport Birkhoff parameterization.
    ...
    def __init__(
        ...
        *,
        ...
        assert num_residual_streams >= 2, "SRTBP2N-mHC requires at least 2 streams"
        assert _is_power_of_two(num_residual_streams), (
            f"SRTBP2N-mHC requires a power-of-2 stream count, got {num_residual_streams}"
        )
    ...
    def _init_hyper_params(self):
        """Initialize SRTBP2N-mHC parameters: H^pre, H^res, H^post, delta."""
    ...
    def _compute_alpha_beta(self, normed: torch.Tensor, device: torch.device):
        ...
        srtbp2n_params = alpha_res[..., : n - 1, : n - 1]

        orig_shape = srtbp2n_params.shape[:-2]
        srtbp2n_flat = srtbp2n_params.reshape(-1, n - 1, n - 1)
...
SRTBP2N_MHC.get_expand_reduce_stream_functions = staticmethod(
    get_init_and_expand_reduce_stream_functions
)
SRTBP2N_MHC.get_init_and_expand_reduce_stream_functions = staticmethod(
    get_init_and_expand_reduce_stream_functions
)


class ScaledPowerOfTwoRecursiveTransportBirkhoff(nn.Module):
    """Standalone wrapper for scaled_recursive_transport_birkhoff_power2()."""
